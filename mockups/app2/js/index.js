...
App.widgets.Menu = {
		...
		render : function(){
			...
			this.model.all.forEach(function  (cur, ind) {
				html += '<span class=menuentity tabindex=' +ind+ '><i class="material-icons" style="color: white; margin-left: 10px;  font-size: 24px;">' + cur  + '</i></span>';
			})
			$('#menu').html(html);
		},
		highlight : function  () {
		...
		this.model.currentList.forEach(function(curId, index){
			// if ( ($epgNowAll[ current.id ] && $epgNowAll[ current.id ].cat.slice(0,1)  == _cat) || _cat === "-1" ){
				...
				html += '<div class="chan" tabindex='+ index + " data-id= "+ curId  + '>' 
					+ '<div class="logochan" style="background-image: url(http://kirito.la.net.ua/tv/_'+ curId + '.jpg)"><img src="' + App.api.img + 'logo/'+ curId + '.png" style="width:100%; margin-top: 20px;">';
					// for favorites
					/** */
					...
					html += '</div><div class="content left">'
					+ '<div class="timeend">'+ stoptime +'</div>'
					+ '<div class="titleprog">'+epg.title +'</div>'
					+ '<div class="textprog">'+epg.text +'</div>'
					;
